...
public class QuotientenkörperAdditiveStruktur<R, GP extends GrothendieckPaar<R>> implements
		Gruppe<GP> {
	Ring<R> ring;
	GrothendieckPaarMenge<R, GP> paarMenge;

	public QuotientenkörperAdditiveStruktur(Ring<R> ring,GrothendieckPaarMenge<R,GP> paarMenge) {
		this.ring = ring;
	...
	public GP getElement(String beschreibung) {
		return neuesPaar(ring.getElement(beschreibung),
				ring.getEins());
	}
	...
	@Override
	public GP getLQuotient(GP r,
			GP re) {
		return op(r, getInverse(re));
	}

	@Override
	public GP getRQuotient(GP r,
			GP li) {
